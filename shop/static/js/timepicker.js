...
function Timepicker(isClk,is24H,isLight,hour,minute){
    ...
    getTmStr = function () {
        let s = "Seleted Time: "
		s +=('0'+(is24H?hour:hour%12==0?12:hour%12)).slice(-2)+':'+('0'+minute).slice(-2);
	...
	create=function(){
		...
		tpick.style.cssText='position:relative;width:240px;top:15px;';
		...
		timeStr.style.cssText='position:absolute;top:265px;width:100%;text-align:left;font:19px Verdana;cursor:default';
		...
		tpick.style.height=isClk?'240px':'300px';
		if(isClk){
	...
	this.getTimeString=function(){return getTmStr1()};
	this.hide=function(){
		if(!isHidden){
			isHidden=!isHidden;
			tpick.style.display='none';
